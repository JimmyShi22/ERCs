...
- **`instanceId: bytes`** — canonical identifier of the deployment instance that emitted this verification, encoded per-chain. On Ethereum: `abi.encodePacked(address(this))`. On Solana: the program address in its canonical encoding. `instanceId` has exactly one job — *which* deployment — and does not encode proof-backend type or scheme; that information is already fully carried by `proofSystem()` (the paradigm, e.g. `"tee/nitro"`, `"zk/sp1"`) and `agentProofProfile` (the exact-config discriminator within that paradigm — circuit version, enclave measurement, committee pubkey — committed as a `bytes32` hash via `proofProfile()`). Overloading `instanceId` with a class/scheme tag would duplicate a distinction the interface already makes elsewhere; keeping it a plain per-chain address keeps each field's job singular — `proofSystem()` answers "what kind," `agentProofProfile` answers "which exact config," `instanceId` answers "which deployment."

- **`chainId: bytes`** — identifies the chain as the UTF-8 byte encoding of its CAIP-2 identifier (`namespace:reference`, e.g. `eip155:1` for Ethereum mainnet). On Ethereum: `bytes(string.concat("eip155:", <decimal-ASCII of block.chainid>))` — see the reference implementation below for a dependency-free conversion. This is a digest-encoding change, not a documentation clarification: `chainId` moves from `abi.encode(block.chainid)` (32-byte, zero-padded `uint256`) to UTF-8 CAIP-2 bytes (variable-length ASCII) — the `bytes` ABI *type* stays the same, but every digest computed after adopting this encoding differs from one computed under the prior wording.

Including `valid`, `agentProofProfile`, `block.timestamp`, `instanceId`, and `chainId` ensures that the same inputs verified by different proof systems, on different chains, by different deployments, at different times, or with different outcomes, produce different digests. `block.timestamp` makes the digest self-contained — the verification time is encoded directly rather than requiring block lookup. `instanceId` and `chainId` prevent cross-deployment and cross-chain collisions when the digest is used as a detached audit anchor.
...
Settlement contracts MUST implement `IAgentVerifiable` to declare which `IAgentVerifier` they use and MUST emit an `AgentVerifierUpdated` event on any change.

Two `AgentVerifierUpdated` event signatures are defined. A conforming deployment MUST emit exactly one of them on each verifier change. The choice belongs to the deployment:

- **Address-based** — for simple address-level tracking. Emits the previous and new verifier addresses.
- **Memory-versioned** — for deployments that record verifier upgrades in an off-chain memory versioning system (e.g. a state-registry that maintains a linear `prevStateRoot → nextStateRoot` chain). Carries a `spaceId` identifying the memory namespace and a `transitionId` identifying the specific transition within that namespace. A consumer can use `spaceId` to locate the full upgrade lineage and `transitionId` to verify this particular switch against it.
...
interface IAgentVerifiable {
    ...
    /// @dev Two variants available. Deployments MUST emit exactly one.
    event AgentVerifierUpdated(
        address indexed previousVerifier,
        address indexed newVerifier
    );

    /// @notice MUST be emitted when the agent verifier changes, with memory-versioned lineage.
    /// @dev Alternative to the address-based variant. Deployments MUST emit exactly one.
    /// @param spaceId      Identifier of the memory namespace holding the upgrade lineage.
    /// @param transitionId Identifier of the transition within that namespace.
    event AgentVerifierUpdated(
        bytes32 spaceId,
...
contract AgentVerifier is IAgentVerifier {
    ...
    function verify(
        ...
        // chainId: UTF-8 bytes of the CAIP-2 identifier, e.g. "eip155:1"
        bytes memory chainId = bytes(string.concat("eip155:", _toDecimalString(block.chainid)));
        ...
        emit VerificationCompleted(
            ...
            agentProofProfile, ts, instanceId, chainId
        );
    }

    /// @dev Minimal decimal-ASCII conversion, dependency-free by design (matches this
    ///      ERC's convention of relative-only imports in reference implementations).
